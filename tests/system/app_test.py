...
class AppTest(TestCase):
    def setUp(self): #zazenemo pred zacetkom vsakega testa da ne ponavljamo kode ker te dve vrstici rabimo velikokrat
    ...
    def test_print_blogs(self):
        with patch('builtins.print') as mocked_print: #replace print function with a mock so we can do the test
           app.print_blogs()
           mocked_print.assert_called_with('- Test by Test Author (0 posts)')

    def test_ask_create_blog(self):
        with patch('builtins.input') as mocked_input:
            mocked_input.side_effect = ('Test', 'Test Author') #side_effect is calling specified values
            app.ask_create_blog()
